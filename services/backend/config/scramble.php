...
return [
    ...
    'info' => [
        ...
        Scrapes product pages from Jumia and Amazon, stores them in MySQL, and serves them here.

        ### Getting a token

        Every endpoint needs one; nothing here is public.

        ```bash
        php artisan api:token frontend
        ```

        Paste it into **Authorize** above and everything on this page becomes runnable.

        ### Worth trying

        - `GET /api/v1/products` twice, and watch the `X-Cache` header go from `MISS` to `HIT`.
        - `POST /api/v1/scrape` with a mix of good and bad URLs. A batch is allowed to partly
          succeed, so the response lists what was queued and what was turned away, with a reason
          for each.
        - `POST /api/v1/scrape` with `https://169.254.169.254/latest/meta-data/`. That is the cloud
          metadata endpoint, and it is refused before any request is made.
        - `GET /api/v1/scrape-jobs` to watch queued work move from pending to completed.
        MARKDOWN,
    ],
     ...
     */
    /*
     * Enabled so the docs page gets an "Authorize" box.
     *
     * Every route sits behind auth:sanctum, and Scramble reads that middleware to document bearer
